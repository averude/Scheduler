...
public interface ScheduleService extends GenericService<WorkDay, Long> {
    @CheckDateParameters
    BasicDto<Employee, WorkDay> findEmployeeScheduleByEmployeeIdAndDate(@Min(value = 1L, message = "{entity.id.size}")
                                                                            Long employeeId,
    ...
    @CheckDateParameters
    List<BasicDto<Employee, WorkDay>> findEmployeeScheduleByDepartmentIdAndDate(@Min(value = 1L, message = "{entity.id.size}")
                                                                                    Long departmentId,
                                                                                @NotNull(message = "{workDay.date.null}")
                                                                                    LocalDate from,
                                                                                    LocalDate to);
    ...
    @CheckDateParameters
    List<BasicDto<Employee, WorkDay>> findEmployeeScheduleByShiftIdAndDate(@Min(value = 1L, message = "{entity.id.size}")
                                                                                   Long shiftId,
                                                                           @NotNull(message = "{workDay.date.null}")
                                                                                   LocalDate from,
                                                                                   LocalDate to);
    ...
    @CheckDateParameters
    List<BasicDto<Employee, WorkDay>> findAllEmployeeScheduleByAuthAndDate(Authentication authentication,
                                                                           LocalDate from,
                                                                           LocalDate to);
